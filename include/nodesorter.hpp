...
struct snode_s
{
  float probability;
  int deg;
  int deg_neigh;             //Sum of neighbors' degree
  int deg_m;
  bool used;
  bool in_candidate;
  node_id id;
...
class VF3NodeSorter:NodeSorter<Node, Edge>
{
  ...
  public:
    vector<node_id> SortNodes(ARGraph<Node,Edge>* pattern);
    vector<node_id> SortNodes(ARGraph<Node,Edge>* pattern, vector<node_id> nodesinmuHi);
...
/**
 * SortNodes with nodesinmuHi
 */

template<typename Node, typename Edge,
typename Probability >
vector<node_id> VF3NodeSorter<Node,Edge,Probability >::SortNodes(ARGraph<Node,Edge>* pattern, vector<node_id> nodesinmuHi)
{
  int nodeCount;
  int i;
  nodeCount = pattern->NodeCount();
  ...
  for(i = 0; i < nodeCount; i++)
  {
	  if(find(nodesinmuHi.begin(), nodesinmuHi.end(), i) == nodesinmuHi.end()){
		  nodes[i].id = i;
		  nodes[i].deg_m = 0; //Dynamically updated
		  nodes[i].deg = pattern->EdgeCount(i);
		  nodes[i].deg_neigh = 0;
  ...
  cout << "node_star.size(): " << node_star.size() << endl;
  ...
  for(auto it=nodes_order.begin();it!=nodes_order.end();++it)
	  cout << "nodes_order: " << *it << endl;
