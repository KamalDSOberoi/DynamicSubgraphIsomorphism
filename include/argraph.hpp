...
public:
	...
	*/
    ...
    //kamal
    ARGraph<Node, Edge> GetInducedSubGraph(NodeVec nVec);
    node_id GetNodeId(Node attri);
};
...
{ assert(node<n);
    ...
    //cout << "out[node].size: " << out[node].size() << endl;
    return out[node][i];
}
...
template<typename Node, typename Edge>
node_id ARGraph<Node, Edge>::GetNodeId(Node attri){
	// get first node_id having attribute attri
	auto it = find(attr.begin(), attr.end(), attri);              // finds first element
	assert(it!=attr.end());                                       // if not found, abort
	return distance(attr.begin(), it);
}
