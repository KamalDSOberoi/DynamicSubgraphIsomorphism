...
*/
//typedef bool (*match_visitor)(int n, node_id c1[], node_id c2[], ARGraph<data_t, Empty> *g1, ARGraph<data_t, Empty> *g2, void* tree_s, void *usr_data);
typedef bool (*match_visitor)(int n, node_id c1[], node_id c2[], void* tree_s, void *usr_data);
           ...
           match_visitor vis, void *usr_data)
{
  ...
  if (s.IsGoal())
  {
    ...
    int n = s.CoreLen();
    s.GetCoreSet(c1, c2);
    return vis(n, c1, c2, &s, usr_data);                   // vis returns false to continue looking for other goal states
  ...
  { //cout << "nextpair true" << endl;
  	  //cout << "n1: " << n1 <<", n2: " << n2 << endl;
	  if (s.IsFeasiblePair(n1, n2))
    {
    State s1(s);                                    // copy constructor for the state; s becomes parent of s1
    s1.AddPair(n1, n2);
    //cout << "pair added: " << n1<<"," <<n2 << endl;
